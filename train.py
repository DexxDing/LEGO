    ...
    with ((torch.set_grad_enabled(True))):
        ...
        for epoch in range(num_epochs):
            ...
            while True:
                ...
                v_features_n, v_features_n2, v_features_n3, t_features_n, labels_n = [n.to(device) for n in batch_n]
                v_features_a, v_features_a2, v_features_a3, t_features_a, labels_a = [a.to(device) for a in batch_a]


                cut = min(v_features_n.shape[0], v_features_a.shape[0])
                v_features_n, v_features_n2, v_features_n3, t_features_n, labels_n = v_features_n[:cut], v_features_n2[:cut], v_features_n3[:cut], \
                    t_features_n[:cut], labels_n[:cut]
                v_features_a, v_features_a2, v_features_a3, t_features_a, labels_a = v_features_a[:cut], v_features_a2[:cut], v_features_a3[:cut], \
                    t_features_a[:cut], labels_a[:cut]


                cut_snippet = min(v_features_n.shape[2], v_features_a.shape[2])
                v_features_n, v_features_n2, v_features_n3, t_features_n = v_features_n[:, :, :cut_snippet, :], \
                                                                            v_features_n2[:, :, :cut_snippet, :],\
                                                                            v_features_n3[:, :, :cut_snippet, :],\
                                                                            t_features_n[:, :, :cut_snippet, :]
                v_features_a, v_features_a2, v_features_a3, t_features_a = v_features_a[:, :, :cut_snippet, :], \
                ...
                # features_list = [v_features, v_features2, v_features3, t_features]

                features_list = [v_features, v_features2, v_features3]  # i3d+swin

                sampled_features = random.sample(features_list, 2)

                feature_1, feature_2 = sampled_features[0], sampled_features[1]

                v_features = feature_1
                t_features = feature_2
