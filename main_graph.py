...
if __name__ == '__main__':
    ...
    class CombinedLoss(nn.Module):
        def __init__(self, k, threshold, w_normal=1., w_var=1.):
            super().__init__()
            self.k = k
    ...
    m = args.m
    n = args.n
    plot_label = str(f"{args.dataset}: m={m}, n={n} av")
    if args.wandb:
        wandb.init(
            project="GraphFusion",
            name=f"{plot_label}_av",
            reinit=True
        )
    model = graphfusion.GraphClassifiction(batch_size=32, m=m, n=n)
    criterion = CombinedLoss(k=5, threshold=0.5)
    optimizer = optim.Adam(model.parameters(),
                           lr=args.lr, weight_decay=args.wd)
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=5, gamma=0.1)

    logging_message_test = f"av setup: m={m}, n={n}, w_var=1., threshold=0.5"
